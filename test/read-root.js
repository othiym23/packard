...
var readRoot = require('../lib/read-root.js')

var basedir = join(__dirname, 'test-read-root')
...
test('read root with one one-track directory, no cue sheet', function (t) {
  ...
  readRoot(basedir).then(
    function (artists) {
      ...
      t.equal(
        album.tracks[0].name,
        '1-Savage_Beatings_for_All.flac',
...
test('read root with one one-track directory, with cue sheet', function (t) {
  ...
  readRoot(basedir).then(
    function (artists) {
      ...
      var album = artist.albums[0]
      t.ok(album, 'found album')
      t.equal(album.name, 'Skiffle_Rumble.flac', "name didn't change")
