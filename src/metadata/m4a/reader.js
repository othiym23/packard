...
function parseITunesGapless (data) {
...
function parseITunesNormalization (data) {
  const parsed = data.trim().split(' ').map(v => parseInt(v, 16))
  return {
  ...
  return throughWatcher
    ...
    .on('finish', () => {
      ...
      if (tags.iTunesNormalization) {
        tags.iTunesNormalization = parseITunesNormalization(tags.iTunesNormalization)
      }
      if (tags.iTunesGaplessData) {
        tags.iTunesGaplessData = parseITunesGapless(tags.iTunesGaplessData)
      }
